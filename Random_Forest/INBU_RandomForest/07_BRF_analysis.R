...
r <- raster("Random_Forest/inbu_data/prediction-surface.tif")
...
occ_pred <- predict(rf, data = ebird_split$train)$predictions[, 2]
...
pd_time <- partial_dependence(rf, 
                              ...
                              data = ebird_split$train) %>% 
  select(time_observations_started, encounter_rate = "1")
...
writeRaster(r_pred, file.path(tif_dir, "rf-model_encounter-rate_inbu.tif"), 
            overwrite = TRUE)
...
title <- "Indigo Bunting Encounter Rate"
